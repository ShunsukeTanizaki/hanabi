...
class Hanabi {
  ...
  update() {
    ...
    if (this.x >> 8 < 0 || this.x >> 8 > SCREEN_SIZE_W || this.y >> 8 > SCREEN_SIZE_H) this.kill = true;
    ...
    if (this.tp == 0 && this.vy >= 0) {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        hanabi.push(new Hanabi(this.x, this.y, this.col, vx, vy, 1, 1, 200));
      }
      let col = rand(0, 3);
      for (let i = -0; i < 100; i++) {
        let r = rand(0, 360);
        ...
        let vx = Math.cos(r * Math.PI / 180) * s;
        let vy = Math.sin(r * Math.PI / 180) * s;
        hanabi.push(new Hanabi(this.x, this.y, col, vx, vy, 1, 1, 200));
      }
    }
    if (this.tp == 1) {
      this.hp--;
  ...
  draw() {
    ...
    zanzo.push(new Zanzo(this.x, this.y, col));
  }
}